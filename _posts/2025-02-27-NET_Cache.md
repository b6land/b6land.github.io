...
### 備註：在舊版 ASP.NET 和 .NET Framework 應用程式的 MemoryCache 類別

這是 .NET Framework 4+ 內建的快取類別，可以用在各種應用程式（Web、桌面、服務）。
...
class Program
{
    static void Main()
    {
        ...
        if (data == null)
        {
            ...
            {
                AbsoluteExpiration = DateTimeOffset.Now.AddMinutes(5) // 快取 5 分鐘
            };
            cache.Set(key, data, policy);
        }

        Console.WriteLine("從快取取出的資料: " + cache[key]);
    }
}
```

如果不需要逾期回收的機制，其實也可以直接用 Dictionary 來代替就好。

- [MemoryCache 類別 (System.Runtime.Caching) - Microsoft Learn](https://learn.microsoft.com/zh-tw/dotnet/api/system.runtime.caching.memorycache?view=net-8.0)
- [使用 .NET Framework 內建的 MemoryCache 來 Cache 常用資料 - Part 1 極簡做法 - Yowko's Notes](https://blog.yowko.com/net-framework-memorycache-1/ )